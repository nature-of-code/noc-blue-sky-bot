...
const noiseSeed = function (seed) {
  noiseLCG.setSeed(seed);
  perlin = new Array(PERLIN_SIZE + 1);
  for (let i = 0; i < PERLIN_SIZE + 1; i++) {
    perlin[i] = noiseLCG.rand();
  }
};

const randomSeed = function (seed) {
  randomLCG.setSeed(seed);
};

const random = function (min, max) {
  const rand = randomLCG.rand();
...
const lerpColor = function (a, b, amount) {
  ...
  const rp = ar + (br - ar) * amount;
  const gp = ag + (bg - ag) * amount;
  const bp = ab + (bb - ab) * amount;
...
export { noise, noiseDetail, noiseSeed, random, randomSeed, lerpColor, nf };
